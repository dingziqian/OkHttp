...
public class OkHttpClient implements Cloneable, Call.Factory, WebSocket.Factory {
  ...
  static {
    Internal.instance = new Internal() {
      ...
      // 获取到的 StreamAllocation,主要处理 Connections，Streams, Calls之间的关系
  ...
  final Dispatcher dispatcher; // 分发请求，内部是有一个ThreadPoolExecutor
  final @Nullable Proxy proxy; // 代理连接，分三种类型直接（DIRECT）、Http(http)、SOCKS。
  final List<Protocol> protocols; // 协议
  final List<ConnectionSpec> connectionSpecs; // 传输层版本和连接协议
  final List<Interceptor> interceptors; // 拦截器
  final List<Interceptor> networkInterceptors; // 网络拦截器
  final EventListener.Factory eventListenerFactory;
  final ProxySelector proxySelector; // 线路选择器，对应Okhttp的一大特点，自行线路选择，找到合适的连接
  final CookieJar cookieJar; // cookie
  final @Nullable Cache cache; // 真正的缓存实现
  final @Nullable InternalCache internalCache; // 内部缓存
  final SocketFactory socketFactory; // socket 工厂
  final @Nullable SSLSocketFactory sslSocketFactory; // Https的支持
